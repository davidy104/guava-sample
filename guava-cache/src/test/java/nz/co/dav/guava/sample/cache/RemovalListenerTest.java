...
public class RemovalListenerTest {
	...
	public void testLoadingCacheExpireAfterWrite() throws Exception {
		...
		LoadingCache<String, TradeAccount> tradeAccountCache = CacheBuilder
				.newBuilder().expireAfterWrite(5L, TimeUnit.MILLISECONDS)
				.maximumSize(5000L).removalListener(removalListener)
	...
	public void testRemovalAfterLastAccessed() throws Exception {
		...
		LoadingCache<String, Book> bookCache = CacheBuilder.newBuilder()
				.expireAfterAccess(10, TimeUnit.MILLISECONDS).softValues()
				.recordStats().removalListener(bookRemovalListener)
		...
		assertThat(bookRemovalListener.getRemovalCause(),
				is(RemovalCause.EXPIRED));
	...
	public void testInvalidateBadValue() throws Exception {
		BookRemovalListener bookRemovalListener = new BookRemovalListener();
		LoadingCache<String, Book> bookCache = CacheBuilder.newBuilder()
				.expireAfterAccess(10, TimeUnit.HOURS).softValues()
				.recordStats().removalListener(bookRemovalListener)
				.build(new CacheLoader<String, Book>() {
					@Override
					public Book load(String key) throws Exception {
		...
		assertThat(bookRemovalListener.getRemovalCause(),
				is(RemovalCause.EXPLICIT));
		assertThat(bookRemovalListener.getRemovedValue().getTitle(),
				is("Be an Architect"));
	}
	...
	public void testRefreshingCacheValues() throws Exception {
		...
		LoadingCache<String, TradeAccount> tradeAccountCache = CacheBuilder
				...
				.removalListener(removalListener).ticker(Ticker.systemTicker())
				.recordStats().build(new CacheLoader<String, TradeAccount>() {
					@Override
					public TradeAccount load(String key) throws Exception {
						return tradeAccountService.getTradeAccountById(key);
					}
